...
Rest.prototype = {
  ...
  findBefore: function(route, req, res, next) {
    ...
    if ( header_value === undefined ) {
      runBefore( route.callbacks["*"].before);
    } else {
      runBefore( route.callbacks["*"].before);
    }
  },
  runBefore: function(before, req, res, next) {
    if (_.isArray(before) ) {
      before.shift()(req,res, next);
  ...
  _createRoute: function(route, app) {
    ...
    app[route.method](route.path, function(req,res,next) {
      self.findController(route, req, res, next);
